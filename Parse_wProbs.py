    ...
    for edusen in F_EDU:
        ...
        if n == 1: #if contains only one edu
            ...
            if prob[0][n-1]==0:
                ...
                writeParse(1, struct[0][n-1], n, prob, rel, struct, edus, 2, F_DP_Sen, eduIndex)
                print >>F_DP_Sen, ")"
...
def parseDocLevel(baseprob, spans, spans_parse, spans_edus, rel_labels, F_FEAT, F_PROB, F_DP): 
    ...
    if n == 1: #contains only one span
        ...
        if e_e_id - b_e_id == 0:#only one edu
            ...
            print >>F_DP, "( Root (span " + str(b_e_id) + " " + str(e_e_id) + ")"
            writeParse(b_e_id, the_struct[0][e_e_id - 1], e_e_id, the_prob, the_rel, the_struct, the_edus, 2, F_DP, 0)
            print >>F_DP, ")"
        ...
        if prob[0][n-1]==0:
            ...
            writeDocLevelParse(1, struct[0][n-1], n, prob, rel, struct, spans, spans_parse, spans_edus, 2, F_DP)
            print >>F_DP, ")"


def writeDocLevelParse(i, k, j, prob, rel, struct, spans, spans_parse, spans_edus, offset, F_DP):
    """ print the parse tree """

    # modifications by Jordon:
    # - added prob as an argument
    # - added this_prob as a variable
    # - added probability values to outputs to F_DP
    ...
    if i < j:
        ...
        if k-i > 0:
            print >>F_DP, offset * " " + "( " + status1 + " (span " + str(i_span[0]) + " " + str(k_span[1]) + ") (rel2par " + rel2par1 + ") (prob " + str(this_prob) + ")"
            writeDocLevelParse(i, struct[i-1][k-1], k, prob, rel, struct, spans, spans_parse, spans_edus, offset+2, F_DP)
            print >>F_DP, offset * " " + ")"

        else:
            writeSentLevelParse(F_DP, offset, status1, i_span, this_prob, rel2par1, spans_parse, spans_edus)

        if j-k-1 > 0:
            print >>F_DP, offset * " " + "( "+ status2 + " (span " + str(int(k_span[1])+1) + " " + str(j_span[1]) + ")" + " (rel2par " + rel2par2 + ") (prob " + str(this_prob) + ")"
            writeDocLevelParse(k+1, struct[k][j-1], j, prob, rel, struct, spans, spans_parse, spans_edus, offset+2, F_DP)
            print >>F_DP, offset * " " + ")"

        else:
            writeSentLevelParse(F_DP, offset, status2, j_span, this_prob, rel2par2, spans_parse, spans_edus)
...
def writeSentLevelParse(F_DP, offset, status1, i_span, prob1, rel2par1, spans_parse, spans_edus):

    # modifications by Jordon:
    # - added prob1 as an argument
    # - added probability values to outputs to F_DP
    ...
    if int(i_span[1]) - int (i_span[0]) == 0:
        print >>F_DP, offset * " " + "( " + status1 + " (leaf " + str(i_span[0]) + ") (rel2par " + rel2par1 + ") (prob " + str(prob1) + ") (text _!" + handleQuote(spans_edus[thisSpan] [0]) + "_!) )"
    
    else:
        print >>F_DP, offset * " " + "( " + status1 + " (span " + str(i_span[0]) + " " + str(i_span[1]) + ") (rel2par " + rel2par1 + ") (prob " + str(prob1) + ")"
        ...
        writeParse(int(i_span[0]) - eI, thisSpanStr[0][n-1], int(i_span[1]) - eI, thisSpanProb, thisSpanRel, thisSpanStr, thisSpanEdus, offset+2, F_DP, eI)
        print >>F_DP, offset * " " + ")"


def writeParse(i, k, j, prob, rel, struct, edus, offset, F_DP, eI):
    """ print the parse tree """

    # modifications by Jordon:
    # - added prob as an argument
    # - added this_prob as a variable
    # - added probability values to non-leaf outputs to F_DP
    ...
    if i < j:
        ...
        if k-i > 0:
            print >>F_DP, offset * " " + "( " + status1 + " (span " + str(eI+i) + " " + str(eI+k) + ") (rel2par " + rel2par1 + ") (prob " + str(this_prob) + ")"
            writeParse(i, struct[i-1][k-1], k, prob, rel, struct, edus, offset+2, F_DP, eI)
            print >>F_DP, offset * " " + " )"
        
        else:
            print >>F_DP, offset * " " + "( " + status1 + " (leaf " + str(eI+i) + ") (rel2par " + rel2par1 + ") (prob " + str(this_prob) + ") (text _!" + handleQuote(edus[i-1]) + "_!) )"
            
        if j-k-1 > 0:
            print >>F_DP, offset * " " + "( " + status2 + " (span " + str(eI+k+1) + " " + str(eI+j) + ")" + " (rel2par " + rel2par2 + ") (prob " + str(this_prob) + ")"
            writeParse(k+1, struct[k][j-1], j, prob, rel, struct, edus, offset+2, F_DP, eI)
            print >>F_DP, offset * " " + " )"
            
        else:
            print >>F_DP, offset * " " + "( " + status2 + " (leaf " + str(eI+j) + ") (rel2par " + rel2par2 +") (prob " + str(this_prob) + ") (text _!" + handleQuote(edus[j-1]) + "_!) )"
