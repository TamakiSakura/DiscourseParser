...
def preprocess(ts_edufile, filename):
    """ create .fmt to extract the dominance sets"""
    
    ts_fmtfile      = "temp/"+filename+"_tmp.fmt"
    ...
    SynFeatures.extractDomSets("temp/"+filename+"_tmp.chp", "temp/"+filename+"_tmp.fmt", filename)#okay
    TopicFeatures.computeLexChains("temp/"+filename+"_tmp.tag")
...
def findFeaturesForUpperDTs(F_W, spans, span_text, span_text_ng, unigram, bigram, trigram, span_text_tag, span_sen, filename):    
    """ find features (just edu nbs) for the upper-level DTs """

    chains = []
    TopicFeatures.read_chains("temp/"+filename+"_tmp.chn", chains) 
...
def extract_features(filename):
    ...
    [domfile, tokfile, edufile, tagfile]  = ["temp/"+filename+"_tmp.dom", "temp/"+filename+"_tmp.tok", "temp/"+filename+"_tmp.edu", "temp/"+filename+"_tmp.tag"]
    [F_DOM, F_TOK, F_EDU, F_TAG]       = [open(domfile, 'r'), open(tokfile, 'r'), open(edufile, 'r'), open(tagfile, 'r')]

    [feat_f_sen, feat_f_doc]   = ["temp/"+filename+"_tmp_sen_par.feat", "temp/"+filename+"_tmp_doc.feat"] #feature files
    ...
    ContFeatures.extract_sent_level_contextual_features("temp/"+filename+"_tmp_sen_par.feat", "temp/"+filename+"_tmp_sen.feat")
    os.unlink("temp/"+filename+"_tmp_sen_par.feat")

    #doc-level
    ret = findFeaturesForUpperDTs(F_W2, spans, span_text, span_text_ng, unigram_doc, bigram_doc, trigram_doc, span_text_tag, span_sen, filename) 
    F_W2.close()
...
def apply_sent_model(test_f, java_classpath, java_prog, filename):
    """ apply the sent-level model to the test_f """
    
    [F_W, F_W2] = [open('temp/'+filename+'_tmp_sen.prob', 'w'), open('temp/'+filename+'_tmp_sen.rel', 'w')]
    subprocess.check_call(['java', '-Xmx4096m', '-classpath', java_classpath, java_prog, '--testing', test_f, '--model-file', 'dcrf.sen.gz'], stdout=F_W, stderr=F_W2)
    F_W.close()
    F_W2.close()
    

def apply_doc_model(test_f, java_classpath, java_prog, filename):
    """ apply the doc-level model to the test_f """
    
    [F_W, F_W2] = [open('temp/'+filename+'_tmp_doc.prob', 'w'), open('temp/'+filename+'_tmp_doc.rel', 'w')]
    subprocess.check_call(['java', '-Xmx4096m', '-classpath', java_classpath, java_prog, '--testing', test_f, '--model-file', 'dcrf.doc.gz'], stdout=F_W, stderr=F_W2)
    F_W.close()
    F_W2.close()
...
def do_parse(infile):
    ...
    filename = os.path.basename(os.path.splitext(infile)[0]).replace("_tmp","")

    preprocess(infile, filename) #okay
    (sen, doc) = extract_features(filename)
    ...
    if sen == "yes":
        apply_sent_model("temp/"+filename+"_tmp_sen.feat", java_classpath, java_prog, filename)
    if doc == "yes":
        apply_doc_model("temp/"+filename+"_tmp_doc.feat", java_classpath, java_prog, filename)

    sen_prob = 'temp/'+filename+'_tmp_sen.prob'
    if not os.path.exists(sen_prob):
        sen_prob = "tmp_sen.prob"

    doc_prob = 'temp/'+filename+'_tmp_doc.prob'
    if not os.path.exists(doc_prob):
        doc_prob = "tmp_doc.prob"

    Parse.parse('parse_sen.rel', sen_prob, "temp/"+filename+"_tmp_sen.feat", "temp/"+filename+"_tmp_sen.dis", 'parse_doc.rel', doc_prob, "temp/"+filename+"_tmp_doc.feat", "temp/"+filename+"_tmp_doc.dis", "temp/"+filename+"_tmp.edu")
