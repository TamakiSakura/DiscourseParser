...
def do_preprocess(infile, filename):
    ...
    W_F  = open("temp/"+filename+"_tmp.tag", 'w')
    ...
    W_F  = open("temp/"+filename+"_tmp.tok", 'w')
    W_F.write(tokenized.strip())
    W_F.close()

    #syntactic parsing
    CHP = "Tools/CharniakParserRerank/parse.sh";
    ch_parsed = subprocess.check_output([CHP, "temp/"+filename+"_tmp.tok"])

    if not ch_parsed:
        print "ERROR IN PARSING .. tmp.tok"
        raw_input(' ')

    W_F  = open("temp/"+filename+"_tmp.chp", 'w')
    W_F.write(ch_parsed.strip())
    W_F.close()
    
    return ["temp/"+filename+"_tmp.tok", "temp/"+filename+"_tmp.tag", "temp/"+filename+"_tmp.chp"]    
...
def extract_features(tok_f, par_f, tag_f, feat_f, word_f, edufile):
...
def do_segment(infile):
    ...
    [tok_f, tag_f, par_f] = do_preprocess(infile, filename)
    has_one_word = extract_features(tok_f, par_f, tag_f, "temp/"+filename+"_tmp.feat", "temp/"+filename+"_tmp.words","temp/"+filename+"_tmp.edu")   

    if has_one_word != -1:
  #      train_model("train.seg")
        pred = apply_model("temp/"+filename+"_tmp.feat")
        processLROutput("temp/"+filename+"_tmp.tok", "temp/"+filename+"_tmp.words", pred, "temp/"+filename+"_tmp.edu") 

        os.unlink("temp/"+filename+"_tmp.words")
        os.unlink("temp/"+filename+"_tmp.feat")
