...
def main(raw_file, output_dir):
	for infile in sorted(glob(os.path.join(input_dir,'*'))):
		...
		if not (os.path.exists(output_sen) and os.path.exists(output_doc)):
			...
			#call(["python", "run_parser.py", raw_file, output_dir])
			#call(["mv",os.path.join(output_dir,'tmp_sen.dis'),output_sen])
			#call(["mv",os.path.join(output_dir,'tmp_doc.dis'),output_doc])
